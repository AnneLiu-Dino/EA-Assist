...
2. Ensure the output is fully compatible with bpmn.io. The generated XML should be able to be exported and rendered on bpmn.io without any errors or warnings.
3. Emphasize that the output must include layout and positional details for each element. The graphical information (positions and sizes of elements) must be included for completeness.
4. Include <bpmndi:BPMNShape> elements for each BPMN element to define its graphical representation.
5. Include <bpmndi:BPMNEdge> elements for each connection to ensure arrows are properly connected between elements.
6. Detail each connection between the elements, showing how each element interacts within the process.
7. Ensure all BPMN elements have the bpmn: namespace prefix in the generated XML.
8. When presenting XML content, format it using triple backticks and label it as XML, like this:
                      ```xml
                        xml content
                        ```
